...
int algorithm(vvc &a,int depth, bool isMaximizer, int alpha, int beta);
...
pair<int,int> bestMove(vector<vector<char>> &a, char c){
    ...
    for(int i=0;i<3;i++){
        for(int j=0;j<3;j++){
            ...
            if(a[i][j]==' '){
                a[i][j]=c;
                curr = algorithm(a,0,false,-100000,100000);
                a[i][j] = ' ';
...
int algorithm(vector<vector<char>> &a,int depth, bool isMaximizer, int alpha, int beta){
    ...
    if(isMaximizer){
        ...
        for(int i=0;i<3;i++){
            for(int j=0;j<3;j++){
                if(board[i][j]==' '){
                    board[i][j]=c;  
                    int score = algorithm(board,depth+1,false,alpha,beta);
                    board[i][j]=' ';
                    best_mx = max(best_mx,score);
                    alpha = max(alpha, best_mx);
                    if(beta <= alpha)
                        break;
                }
            }
        }
        return best_mx-depth;       // depth factor is basically to add penalty in delayed wins 
    }else{
        int best_mn = INT_MAX;
        ...
        for(int i=0;i<3;i++){
            for(int j=0;j<3;j++){
                if(board[i][j]==' '){
                    board[i][j]=p;  
                    int score = algorithm(board,depth+1,true,alpha, beta);
                    board[i][j]=' ';
                    best_mn = min(best_mn,score);
                    beta = min(beta,best_mn);
                    if(beta<=alpha)
                        break;
                }
            }
